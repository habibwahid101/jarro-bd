    ...
    >
      <div className={`relative w-full ${aspectRatio} overflow-hidden rounded-md bg-[#FBE8E4] border border-[#F0D9DC]/80 transition-all duration-300 group-hover:border-[#E8B9C1] group-hover:shadow-md`}>
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5 z-10">
          {product.isNew && (
            <span className="text-[9px] uppercase tracking-[0.15em] font-bold px-2 py-0.5 bg-[#241A1E] text-[#FDF4F1] rounded-xs shadow-xs">New</span>
          )}
          {product.isBestSeller && !product.isNew && (
            <span className="text-[9px] uppercase tracking-[0.15em] font-bold px-2 py-0.5 bg-[#C2607D] text-white rounded-xs shadow-xs">Best Seller</span>
          )}
          {discountPercent > 0 && (
            <span className="text-[9px] uppercase tracking-[0.15em] font-bold px-2 py-0.5 bg-[#8A2424] text-white rounded-xs shadow-xs">Save {discountPercent}%</span>
          )}
          {product.stock <= 5 && product.stock > 0 && (
            <span className="text-[9px] uppercase tracking-[0.15em] font-medium px-2 py-0.5 bg-amber-800 text-white rounded-xs shadow-xs">Only {product.stock} Left</span>
          )}
        </div>
        <button
        ...
        <div className="absolute bottom-2.5 inset-x-2.5 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
          ...
          >
            {isAdding ? (
              <><Check className="w-3.5 h-3.5 text-emerald-400" /><span>Added</span></>
            ) : (
              <><ShoppingBag className="w-3.5 h-3.5" /><span>{product.stock > 0 ? 'Quick Add' : 'Sold Out'}</span></>
            )}
          </button>
        </div>
      </div>
      <div className="mt-3 flex flex-col space-y-1">
        <div className="flex items-center justify-between">
          ...
          {product.clothingSpecs && (
            <span className="text-[10px] text-[#A8828A] tracking-wider">{product.clothingSpecs.fabric}</span>
          )}
        ...
        <h3 className="text-sm font-medium text-[#241A1E] line-clamp-1 group-hover:text-[#C2607D] transition">{product.name}</h3>
        <p className="text-[11px] text-[#8C6A72] line-clamp-1">{product.subtitle}</p>
        <div className="flex items-baseline gap-2 pt-0.5">
          <span className="text-sm font-semibold text-[#241A1E]">{formatBDT(product.price)}</span>
          {product.oldPrice && product.oldPrice > product.price && (
            <span className="text-xs text-[#B98C93] line-through">{formatBDT(product.oldPrice)}</span>
          )}
        </div>
      </div>
    </div>
